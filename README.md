...
```
-  列の重複無く「語彙の名称」列～「語彙の作成者」列全てが必須です
-  「語彙の名称」列～「語彙の作成者」列以外の列を含んでいても問題ありません
...
```
-  列の重複無く「用語名」列～「確定済み用語」列全てが必須です
-  「用語名」列～「確定済み用語」列以外の列を含んでいても問題ありません
-  「用語名」列
    -  分野の用語
...
-  「言語」列
    ...
    -  空白の場合は、ファイル読み込み時に"ja"として処理されます
-  「代表語のURI」列
    -  各代表語に対するユニークなID
...
-  「他語彙体系の同義語のURI」列
    ...
    -  同義語についてはそれらの行に対して同じ値を記述してください
    -  他の語彙で同義語が存在する場合に記述してください
-  「用語の説明」列
    -  その用語あるいはその同義語についての説明
    -  同義語についてはそれらの行に対して「言語」ごとに同じ値を記述してください
    -  各用語について説明書きが必要な場合に記述してください
-  「作成日」列
...
-  「x座標値」列
    ...
    -  編集用語彙作成手順に従って再計算することが可能です
    -  数値以外の値が記述されている場合は、ファイル読み込み時に"0"として処理されます
-  「y座標値」列
...
```
-  列の重複無く「用語名」列～「y座標値」列全てが必須です
-  「用語名」列～「y座標値」列以外の列を含んでいても問題ありません
